...
class HandRepository:
    ...
    def get_hand_by_id(self, hand_id: str) -> Hand | None:
        ...
        with self.db_conn.cursor() as cursor:
            cursor.execute(
                ...
                SELECT
                id, has_ended,
                number_of_players, small_blind_idx,
                big_blind_idx, dealer_idx,
    ...
    def create_hand(self, hand: Hand) -> Hand:
        ...
        with self.db_conn.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO hands (
                has_ended, number_of_players,
                small_blind_idx, big_blind_idx,
    ...
    def update_hand(self, hand: Hand) -> Hand:
        ...
        with self.db_conn.cursor() as cursor:
            cursor.execute(
                """
                UPDATE hands
                SET has_ended = %s, number_of_players = %s,
                small_blind_idx = %s, big_blind_idx = %s,
    ...
    def get_hands_by_status(self, status: bool) -> list[Hand]:
        ...
        with self.db_conn.cursor() as cursor:
            cursor.execute(
                """
                SELECT id, has_ended, number_of_players,
                small_blind_idx, big_blind_idx, dealer_idx,
                stack_size, big_blind_size
                FROM hands
                WHERE has_ended = %s;
            """,
