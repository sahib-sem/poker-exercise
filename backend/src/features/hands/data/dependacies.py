...
def get_player_repository(db_conn: psycopg2.extensions.connection = Depends(get_db_conn)) -> PlayerRepository:
    return PlayerRepository(db_conn)

def get_action_repository(db_conn: psycopg2.extensions.connection = Depends(get_db_conn)) -> ActionRepository:
    return ActionRepository(db_conn)

def get_hand_repository(db_conn: psycopg2.extensions.connection = Depends(get_db_conn), action_repo: ActionRepository = Depends(get_action_repository), player_repo: PlayerRepository = Depends(get_player_repository)) -> HandRepository:
